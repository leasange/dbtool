...
namespace Ls.SyntaxHighlighting
{
    public partial class SHTextBox : RichTextBox
    {
        public const int maxHighlightTextLength = 655350;
        public const int maxAutoHighlightTextLength = 163840;
        ...
        private SyntaxType _syntaxType = SyntaxType.SQL;
        ...
        /// 设置语法类型
        /// </summary>
        public Ls.SyntaxHighlighting.SyntaxType SyntaxType
        {
        ...
        private void UpdateSyntaxType()
        {
            ...
            {
                case SyntaxType.SQL:
                    sr = new StreamReader(this.GetType().Assembly.GetManifestResourceStream("Ls.SyntaxHighlighting.SyntaxXml.SQLReservedWords.xml"));
                    break;
                case SyntaxType.CSharp:
                    sr = new StreamReader(this.GetType().Assembly.GetManifestResourceStream("Ls.SyntaxHighlighting.SyntaxXml.CSharpReservedWords.xml"));
                    break;
                default:
                    break;
            }
            ...
            sr.Dispose();
            LoadCustomSyntaxTypeXmlType(str);
        }
        internal string GetXmlAttribute(XmlNode node, string attriName)
        {
            try
            {
        ...
        internal XmlNodeList GetNodeList(XmlDocument doc, string xpath)
        {
            try
            {
        ...
        internal FontStyle GetFontStyle(string fontStyles)
        {
            FontStyle fonts = FontStyle.Regular;
            try
            {
                if (fontStyles != null && fontStyles.Trim() != "")
                {
                    string[] fss = fontStyles.Trim().Split('|');
                    foreach (string s in fss)
                    {
                        try
                        {
                            FontStyle fs = (FontStyle)Enum.Parse(typeof(FontStyle), s, true);
                            fonts |= fs;
                        }
                        catch (Exception)
                        {
        ...
        internal Font GetFont(string fontStyles)
        {
            return new Font(this.Font, GetFontStyle(fontStyles));
        }
        internal Font GetFont(FontStyle fs)
        {
        ...
        internal DescriptorType GetDesType(string type)
        {
            try
            {
                return (DescriptorType)Enum.Parse(typeof(DescriptorType), type, true);
            }
            catch (Exception)
            {
        ...
        internal bool GetAutoComplete(string auto)
        {
            bool res = true;
            bool.TryParse(auto, out res);
            return res;
        }
        /// <summary>
        /// 加载自定义语法
        ...
        public void LoadCustomSyntaxTypeXmlType(string xml)
        {
            this.WordWrap = false;
            ...
            this.HighlightDescriptors.Clear();
            #region 分割符
            XmlNodeList list = GetNodeList(doc, "Syntax/Seperators/Seperator");
            if (list != null)
            {
                ...
                {
                    string chs = item.Attributes["text"].Value;
                    if (dics.ContainsKey(chs))
                    {
                        this.Seperators.Add(dics[chs]);
                    }
                    else
                    {
                        this.Seperators.Add(chs[0]);
                    }
                }
            }
            #endregion
            #region 关键字
            list = GetNodeList(doc, "Syntax/ReservedWords/ReservedWord");

            if (list != null)
            {
                XmlNodeList listp = GetNodeList(doc, "Syntax/ReservedWords");
                Font font = new Font(this.Font, FontStyle.Bold | FontStyle.Italic);
                if (listp.Count > 0)
                {
                    string fonts = GetXmlAttribute(list[0], "fontstyle");
                ...
                foreach (XmlNode item in list)
                {
                    string text = GetXmlAttribute(item, "text");
                    if (text == null || text.Length == 0)
                    {
                        continue;
                    }
                    string type = GetXmlAttribute(item, "type");
                    this.AddHighlightDescriptor(DescriptorRecognition.WholeWord, text, DescriptorType.Word, Color.Blue, font, true);
                }
            }
            #endregion
            #region 开始结束字符串
            /* list = GetNodeList(doc, "Syntax/Descriptors/Descriptor");
            if (list != null)
            {
                foreach (XmlNode item in list)
                {
                    string start = GetXmlAttribute(item, "start");
                    if (start == null || start.Length == 0)
                    {
                        continue;
                    }
                    string end = GetXmlAttribute(item, "end");
                    DescriptorType dt = DescriptorType.ToCloseToken;
                    if (end == null || end.Length == 0)
                    {
                        dt = DescriptorType.ToEOL;
                    }
                    string fontstyle = GetXmlAttribute(item, "fontstyle");
                    this.AddHighlightDescriptor(DescriptorRecognition.StartsWith, start, dt, end, Color.Green, GetFont(fontstyle), false);
                }
            }*/

            list = GetNodeList(doc, "Syntax/TextBeginEnds/TextBeginEnd");
            if (list != null)
            {
                foreach (XmlNode item in list)
                {
                    ...
                    DescriptorType dt = DescriptorType.None;

                    dt = GetDesType(type);

                    if (dt == DescriptorType.None)
                    {
                        continue;
                    }
                    string strcolor = GetXmlAttribute(item, "color");
                    string fontstyle = GetXmlAttribute(item, "fontstyle");
                    string autocom = GetXmlAttribute(item, "autocomplete");

                    this.AddHighlightDescriptor(DescriptorRecognition.StartsWith, start, dt, end, GetColor(strcolor), GetFont(fontstyle), GetAutoComplete(autocom));
                }
            }
            #endregion
            #region 文本检测
            list = GetNodeList(doc, "Syntax/Texts/Text");
            if (list != null)
            {
                foreach (XmlNode item in list)
                {
                    ...
                    string type = GetXmlAttribute(item, "type");
                    DescriptorType dt = DescriptorType.ToEOW;
                    try
                    {
                        dt = (DescriptorType)Enum.Parse(typeof(DescriptorType), type, true);
                    }
                    catch (Exception)
                    {
                        continue;
                    ...
                    string color = GetXmlAttribute(item, "color");
                    Color clor = Color.Black;
                    try
                    {
                        clor = Color.FromName(color);
                    }
                    catch (Exception)
                    {
                    ...
                    {
                        case DescriptorType.Word:
                            this.AddHighlightDescriptor(DescriptorRecognition.WholeWord, text, DescriptorType.Word, clor, GetFont(fontstyle), true);
                            break;
                        case DescriptorType.WordRegEx:
                            this.AddHighlightDescriptor(DescriptorRecognition.RegEx, text, DescriptorType.Word, clor, GetFont(fontstyle), false);
                            break;
                        case DescriptorType.ToEOL:
                            break;
                        case DescriptorType.ToCloseToken:
                            break;
                        case DescriptorType.ToEOW:
                            this.AddHighlightDescriptor(DescriptorRecognition.StartsWith, text, DescriptorType.ToEOW, clor, GetFont(fontstyle), true);
                            break;
                        default:
                            break;
                    }
            ...
            // this.AddHighlightDescriptor(DescriptorRecognition.RegEx, sEx, DescriptorType.Word, Color.Red, tmp, false);
            //  tmp = new Font(Font, FontStyle.Bold);
            //highlight numbers, 
            //  this.AddHighlightDescriptor(DescriptorRecognition.RegEx, "\\b(?:[0-9]*\\.)?[0-9]+\\b", DescriptorType.Word, Color.Magenta, tmp, false);
        }
                ...
                case Win32.WM_KEYDOWN:
                    {
                        ...
                        else
                        {
                            if (((Keys)(int)m.WParam == Keys.Space) &&
                                ((Win32.GetKeyState(Win32.VK_CONTROL) & Win32.KS_Alt) != 0))
                            {
                                CompleteWord();
                            }
                ...
                case Win32.WM_CHAR:
                    {
                        ...
                        {
                            case Keys.Space:
                                if ((Win32.GetKeyState(Win32.VK_CONTROL) & Win32.KS_Alt) != 0)
                                {
                                    return;
                                }
                                break;
